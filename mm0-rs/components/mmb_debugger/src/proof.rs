...
use crate::sorts_compatible;
use crate::unify::UMode;
use crate::{ MmbState, MmbItem, MmbExpr };
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Finish<'b> {
    Thm(&'b MmbItem<'b>, bool),
    Unfold(&'b MmbItem<'b>, &'b MmbItem<'b>),
}
...
impl<'b, 'a: 'b> MmbState<'b, 'a> {
    pub fn run_proof(
        ...
        for maybe_cmd in proof {
            ...
            self.try_snapshot(Either::L(cmd));
            match cmd {
    ...
    fn proof_dummy(&mut self, sort_num: SortId) -> Res<()> {
        ...
        make_sure!(self.mmb_file.sort(sort_num).unwrap().0 & crate::SORT_STRICT == 0);
    ...
    fn proof_hyp(
        ...
        make_sure!(e_sort_mods & crate::SORT_PROVABLE != 0);
        self.hstack.push(e);
    ...
    fn proof_unfold(&mut self) -> Res<()> {
        ...
        self.run_unify(
            crate::unify::UMode::UDef,
            none_err!(self.mmb_file.term(*term_num))?.unify(),
            e_prime,
            Some(Finish::Unfold(f_ebar, e_prime))
        )?;
